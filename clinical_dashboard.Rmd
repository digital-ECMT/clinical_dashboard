...
measurements_heatmap <- reactive({
   measurements %>%
    ...
    ggplot(aes(x=SUBJECT_ID, y=MEASUREMENT_DATETIME, fill=as.numeric(MEASUREMENT_VALUE))) +
    geom_tile() +
    coord_flip() +
    labs(title = paste0("Changes in measurements"), x="Datetime", y = "Patient", fill = input$selectedMeasurement)
})
