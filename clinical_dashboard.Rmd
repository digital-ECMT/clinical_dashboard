...
output$normal_range_slider <- renderUI({
   sliderInput("normal_range", 
               label = "Normal range:", 
               min = min(as.numeric(measurements$MEASUREMENT_VALUE[measurements$MEASUREMENT_NAME==input$selectedMeasurement])),
               max = max(as.numeric(measurements$MEASUREMENT_VALUE[measurements$MEASUREMENT_NAME==input$selectedMeasurement])),
               value = c(quantile(as.numeric(measurements$MEASUREMENT_VALUE[measurements$MEASUREMENT_NAME==input$selectedMeasurement]))[2],
                         quantile(as.numeric(measurements$MEASUREMENT_VALUE[measurements$MEASUREMENT_NAME==input$selectedMeasurement]))[4]
                         )
   )
})
...
measurements_heatmap <- reactive({
  measurements %>%
    ...
    dplyr::filter(MEASUREMENT_NAME == input$selectedMeasurement) %>%
    ggplot(aes(x=SUBJECT_ID, y=difftime(MEASUREMENT_DATETIME, get(input$selectedBaseline), unit="days"))) +
    geom_tile(aes(colour=as.numeric(MEASUREMENT_VALUE), alpha=0.5)) +
    coord_flip() +
    labs(title = paste0("Changes in measurements"), x="Patient", y ="Time, days" , colour = input$selectedMeasurement) +
    guides(alpha="none") +
...
fillCol(height = 600, flex = c(NA, 1),
  inputPanel(
    selectInput(inputId = "selectedBaseline", label="Select baseline event:", choices = event_names, selected = event_names[1], multiple = FALSE,  selectize = TRUE, width = NULL, size = NULL),
    selectInput(inputId = "selectedMeasurement", label="Select measurement:", choices = measurement_names, selected = measurement_names[1], multiple = FALSE,  selectize = TRUE, width = NULL, size = NULL),
