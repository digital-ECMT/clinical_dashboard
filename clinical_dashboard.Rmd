...
#     ggplot(aes(x=SUBJECT_ID, y=difftime(MEASUREMENT_DATETIME, ADMISSION, unit="days"), fill=as.numeric(MEASUREMENT_VALUE))) +
#     geom_tile() +
#     coord_flip() +
#     labs(title = paste0("Changes in measurements"), y="Time, days", x = "Patient", fill = "urea")
measurements_heatmap <- reactive({
   measurements %>%
    dplyr::filter(SUBJECT_ID %in% selected_patients()) %>%
    dplyr::filter(MEASUREMENT_NAME == input$selectedMeasurement) %>%
    ggplot(aes(x=SUBJECT_ID, y=difftime(MEASUREMENT_DATETIME, get(input$selectedBaseline), unit="days"), fill=as.numeric(MEASUREMENT_VALUE))) +
    geom_tile() +
    coord_flip() +
    labs(title = paste0("Changes in measurements"), x="Patient", y ="Time, days" , fill = input$selectedMeasurement)
})


## get this to work in order to set colour breaks... 
# measurements %>%
#     dplyr::filter(SUBJECT_ID %in% selected_patients) %>%
#     dplyr::filter(MEASUREMENT_NAME == "UREA") %>%
#     ggplot(aes(x=SUBJECT_ID, y=difftime(MEASUREMENT_DATETIME, get("ITU"), unit="days"))) +
#     geom_tile(aes(colour=as.numeric(MEASUREMENT_VALUE))) +
#     coord_flip() +
#     labs(title = paste0("Changes in measurements"), y="Time, days", x = "Patient", colour = "urea") +
#     scale_colour_steps2(breaks = c(5,10),
#         low = "blue",
#         mid = "green",
#         high = "red",
#         midpoint = 8,
