...
# demographics_wide %>%
#     dplyr::mutate(day = round(difftime(time2 = get("ADMISSION"), time1 = get("DIED"), unit="days"), digits = 0)) %>%
#     ggplot(aes(x=day, group=get("GENDER"), color=get("GENDER"))) +
#     stat_ecdf(geom = "step") +
#     scale_color_manual(values = dECMTpalette) +
#     labs(title = "title", x="Days since admission", y="proportion", fill = "GENDER", color = "GENDER") +
#     expand_limits(x=0)

## get cumulative proportion per group vs days since baseline 
# demographics_wide %>%
#     group_by(get("GENDER")) %>%
#     mutate(day = round(difftime(time2 = get("ADMISSION"), time1 = get("DISCHARGED"), unit="days"), digits = 0)) %>%
#     arrange(day) %>%
#     mutate(discharges = cumsum(!is.na(DISCHARGED))) %>%
#     mutate(group_total = length(unique(SUBJECT_ID))) %>%
#     mutate(cum_prop = round(discharges/group_total, digits = 3)) %>%
#     ggplot(aes(x=day, y=cum_prop, color=GENDER)) +
#         geom_step() +
#         expand_limits(x=0, y=c(0,1)) +
#         scale_color_manual(values = dECMTpalette) +
#         labs(title = "title", x="Days since ADMISSION", y="Proportion with DISCHARGE event", fill = "GENDER", color = "GENDER")  
