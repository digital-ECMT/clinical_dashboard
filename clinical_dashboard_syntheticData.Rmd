...
for(i in 1:length(subject_ids)) {
  ...
  for(j in 1:length(inpatient_dates)) {
    ...
    if(cancer_type == "PANCREAS") {value <- value * 3}
    ...
    if(final_event == "DIED") { value <- value * rnorm(n=1, mean = 1.5, sd=0.1)}
    if(gender == "MALE") {value <- value * rnorm(n=1, mean = 1.5, sd=0.1)}
    value <- round(value, digits = 1)
    measurements_synth <- measurements_synth %>%
      add_row(
        SUBJECT_ID = subject_id, 
    ...
    if(datetime > treatment_start &  final_event == "DISCHARGED") { value <- value - 10}
    ...
    if(final_event == "DIED") { value <- value * rnorm(n=1, mean = 0.85, sd=0.1)}
    if(gender == "MALE") {value <- value * rnorm(n=1, mean = 0.85, sd=0.1)}
    value <- round(value, digits = 1)
    measurements_synth <- measurements_synth %>%
      add_row(
        SUBJECT_ID = subject_id, 
    ...
    if(final_event == "DIED") { value <- value * 1.5}
    value <- round(value, digits = 1)
    measurements_synth <- measurements_synth %>%
      add_row(
        SUBJECT_ID = subject_id, 
